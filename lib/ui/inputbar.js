...
var config = require('../config')
  , session = require('../app')
  , { safeGet, getPersonaState } = require('../helpers');
...
function idleTimer() {
    ...
    if (session.connected) {
        ...
        if (personaState !== 'online') {
            const target = ui.steam.Steam.EPersonaState.Online;
            ui.steam.friends.setPersonaState(target);
            session.idle_state = target;
        }
    }
    ...
    session.idle_timeout = setTimeout(() => {
        if (session.connected) {
            ...
            const target = ui.steam.Steam.EPersonaState.Away;
            ui.steam.friends.setPersonaState(target);
            session.idle_state = target;
            ...
            session.idle_timeout = setTimeout(() => {
                if (session.connected) {
                    ...
                    const target = ui.steam.Steam.EPersonaState.Snooze;
                    ui.steam.friends.setPersonaState(target);
                    session.idle_state = target;
