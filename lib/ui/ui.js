...
const handleSteam = (steam) => {
    ...
    steam.on('idle', () => {
        ...
        this.inputbar.idleTimer();
    });

    steam.on('personaUpdate', (state) => {
    // Your persona updated
        this.statusbar.statusUpdate('n' + state)
    });

    steam.on('friendUpdate', () => {
    // A friend you're in pm with updated their persona
        this.statusbar.statusUpdate('c');
    });
