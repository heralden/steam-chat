...
module.exports = (args) => {
    switch (args[0]) {
        ...
        case CMD.persona:     return personaNameWrapper(args);
        ...
        case CMD.help:        return helpWrapper(args);
...
const getWrapper = args => verify(logger,
    ...
    () => {
        if (config.isValidKey(args[1])
            ...
            logger.log('debug', [typeof(res), msg].join(': '), val);
    ...
    () => {
        switch (args[1]) {
            ...
            case 'run':
                ...
                break;
            default:
                logger.log('info', doc.cmd.autojoinNoAction, args[0]);
        }
    }
)
...
function autojoinList([ cmd, action, val ]) {
}

function autojoinRun([ cmd, action, val ]) {
}
