...
var { verify, checker, validator, safeGet,
...
module.exports = (args) => {
    switch (args[0]) {
        ...
        case CMD.accept:      return acceptFriendWrapper(args);
        ...
        case CMD.games:       return gamesHandler();
        ...
        case CMD.dump:        return dumpHandler(args);
...
function setFriends() {
    let friendsObj = {};
    session.friends.forEach((ID) => {
        friendsObj[ID] = safeGet(session.users[ID], 'player_name');
    });
    config.set('friends', friendsObj);
}
...
function gamesHandler() {
    session.show_games = session.show_games ? false : true;
...
function dumpHandler([ cmd, arg ]) {
    let target = session;
    if (arg === 'friends')
        target = ui.steam.friends;

    const file = require('path')
        .join(__dirname, '..', '..', 'dump.log');

    require('fs').writeFile(
        file, 
        require('util').inspect(target, { depth: null }), 
        'utf8', 
        (err) => {
            if (err) logger.log('error', "%s: error: ", cmd, err);
        }
    );
}
...
function autojoinAdd([ cmd, _action, val ]) {
    const add = (target) => {
...
function autojoinRemove([ cmd, _action, val ]) {
...
const acceptFriendWrapper = ([ cmd ]) => verify(logger,
    checker(
        vals.isConnected()
    ),
    () => {
        const f = ui.steam.friends.friends;
        const id = Object.keys(f).filter(ID => 
            f[ID] === ui.steam.Steam.EFriendRelationship.RequestRecipient
        );

        if (id.length > 0) {
            const names = id.map(ID => 
                safeGet(session.users[ID], 'player_name') || ID
            );

            ui.userwin.listSelect(names, item => {
                if (item) {
                    const index = id[names.indexOf(item)];
                    ui.steam.friends.addFriend(index);
                    logger.log('info', doc.msg.acceptedFriendRequest, cmd, item);
                }
                ui.inputbar.read();
            });
        } else {
...
const statusWrapper = args => verify(logger,
    ...
    () => {
        ...
        if (status in ui.steam.Steam.EPersonaState && 
            ...
            // set idle_state back to online in case you were idle
            session.idle_state = ui.steam.Steam.EPersonaState.Online;
