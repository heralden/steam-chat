...
function acceptFriendHandler(arg) {
    ...
    var friends = steam.friends.friends;

    var id = Object.keys(friends).filter(
        ID => friends[ID] === steam.Steam.EFriendRelationship.RequestRecipient
    );
    ...
    userwin.listSelect(name, item => {
        steam.friends.addFriend(id[name.indexOf(item)]);
...
function leaveChatHandler(arg) {
    if (session.currentChat.length === CHATIDLEN) {
        if (session.connected) {
            steam.friends.leaveChat(session.currentChat);
    ...
    chatwin.deleteChat(session.currentChat);
} 
...
const joinChatWrapper = args => verify(
    checker(
        ...
        vals.isConnected(),
        checkCond(
            args[1].length !== CHATIDLEN && 
                session.lastInvite.length !== CHATIDLEN,
