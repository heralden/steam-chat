...
function current() {
    ...
    if (currChat === "log")
        ...
        var color = "cyan-fg";
        const state = session.users[currChat].persona_state
        if (state !== 'online') { // blue-fg won't be visible due to blue-bg
            color = getPersonaColor(state);
        }

        const nick = session.users[currChat].player_name;
        currName = `{${color}}${nick}{/${color}}`;
    }
...
// w{0|1|X}{ID}
// Remove, add or check ID for isTyping partner in PM
function write(arg) {
    const add = arg.substring(0, 1)
        , ID = arg.substring(1);

    if (Number(add) === 1) { // Add

        if (!stats.write.includes(ID))
            stats.write.push(ID);

        stats.writeTimeout[ID] = setTimeout(() => {
            statusUpdate('w0' + ID);
        }, 15*1000);

    } else if (Number(add) === 0) { // Remove
        stats.write = stats.write.filter((e) => e !== ID);
        clearTimeout(stats.writeTimeout[ID]);
    }

    if (stats.write.includes(session.currentChat))
