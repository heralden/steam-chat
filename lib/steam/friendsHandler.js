...
module.exports = {
    ...
    // Message in group chat
        ...
        clearTimeout(session.ghost_timer);
        session.ghost_timer = setTimeout(checkIfGhostChat(chatID), config.get('ghostcheck'));
        ...
        switch (EChatRoomEnterResponse) {
            case res.Success: 
                ...
                for (var steamID in e) {
                    session.clans[chatID].members[steamID] = e[steamID].rank;
                    count++;
                }
    ...
    // Something happened in group chat/clan.
        ...
        if (stateChange == "entered") {
            ...
            clan.members[steamID] = steamFriends.chatRooms[chatID][steamID].rank;
            clan.user_live++;
...
function getUserState(steamID) {
    var userState = getPersonaState(steamFriends.personaStates[steamID].persona_state);
    if (steamFriends.personaStates[steamID].game_name.length > 0)
        userState = "ingame";
    return userState;
}

function getPersonaState(persona) {
    var e = Steam.EPersonaState;
    switch (persona) {
