...
module.exports = {
    ...
    // Message from friend
        ...
        if (msg.length == 0)
            steam.emit('isTyping', steamID, user, getUserState(steamID));
        else
            steam.emit('message', steamID, user, getUserState(steamID), msg);
    },
    ...
    // Message in group chat
        ...
        steam.emit('groupMessage', chatID, user, getUserState(steamID), msg);
    },
    ...
    // Something happened in group chat/clan.
        ...
        steam.emit('activity', chatID, stateChange, getUserName(steamID), getUserName(actorID));
