...
var session = require('../app')
  , { safeGet, replaceString, getPersonaState } = require('../helpers');
...
module.exports = {
    ...
    friendMsgEchoHandler(steamID, msg, EChatEntryType) {
    // Message you sent to a friend from a different client
    // Note: steamID is your friend, not your own.
        const user = session.steamNick;
        const persona = safeGet(session.users[session.steamID], 'persona_state');

        logger.log('debug', "friendMsgEchoToSender event", { steamID, EChatEntryType });
    ...
    // Activity in friends list.
        ...
        switch (EFriendRelationship) {
            case fr.None: // steamID has removed you as a friend
                ...
                if (session.users[steamID].persona_state === 'offline')
            ...
            case fr.Friend: // steamID is now your friend
                if (!session.friends.includes(steamID))
    ...
    // Emits once on login. Check if there are pending friend requests and set which users are your friends. this.friends.friends is now populated.
        ...
        const friends = this.friends.friends;
        const fr = this.Steam.EFriendRelationship;
        const friendsObj = config.get('friends');
        ...
        for (let steamID in friends) {
            ...
            switch (friends[steamID]) {
                ...
                case fr.Friend: // your friend
                    ...
                    // Populate offline friends with names from config.json
                    if (session.users[steamID] === undefined &&
                        friendsObj[steamID] !== undefined) {
                        session.users[steamID] = {
                            persona_state: 'offline',
                            player_name: friendsObj[steamID],
                            game_name: ''
                        };
                    }
                ...
                default:
                    logger.log('debug', "Unhandled EFriendRelationship", friends[steamID]);
            }
        }
        this.emit('listUpdate');
    }
...
function getRank(ClanPermission) {
    ...
    switch (ClanPermission) {
        case e.Owner:           
        case e.Officer:         
        case e.OwnerAndOfficer:
        case e.OwnerOfficerModerator: 
        case e.AllMembers:      return "admin";
