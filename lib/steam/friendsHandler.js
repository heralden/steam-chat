...
module.exports = {
    ...
    // Message from friend
        ...
        if (msg.length == 0)
            this.emit('isTyping', steamID, user, getUserState.call(this, steamID));
        else
            this.emit('message', steamID, user, getUserState.call(this, steamID), msg);
    },
    ...
    // Message in group chat
        ...
        this.emit('groupMessage', chatID, user, getUserState.call(this, steamID), msg);
    },
    ...
    // Someone has made changes to their persona
        ...
        session.users[friend.friendid] = {
            persona_state: getPersonaState.call(this, friend.persona_state, friend.game_name),
    ...
    // Something happened in group chat/clan.
        ...
        if (MemberStateChange === this.Steam.EChatMemberStateChange.Entered) {
            ...
            clan.members[steamID] = 
                getRank.call(this, this.friends.chatRooms[chatID][steamID].rank);
            clan.user_live++;
...
function getUserState(steamID) {
    var friend = this.friends.personaStates[steamID];
    return getPersonaState.call(this, friend.persona_state, friend.game_name);
}
...
function getStateChangeText(MemberStateChange, youBool, user, actor) {
    ...
    switch (MemberStateChange) {
        ...
        case e.Kicked:
            if (!youBool)
                return `${user} was kicked by ${actor}.`;
            else
                return `You have been kicked from the chat by ${actor}.`;
        case e.Banned:
            if (!youBool)
                return `${user} was banned by ${actor}.`;
            else
                return `You have been banned from the chat by ${actor}.`;
        default:                
            return null;
    }
}
