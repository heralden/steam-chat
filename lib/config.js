...
class Config {
    ...
    constructor(configPath) {
        ...
        this._checkConfig(configPath);
    }

    setPath(configPath) {
        this.configFile = configPath;
        logger.log('debug', "Config path has been changed: %s", configPath);
        this._checkConfig(configPath);
    }

    _checkConfig(configPath) {
        fs.stat(configPath, (err) => {
            if (err)
                logger.log('debug', "File %s stat failure.", configPath, err);
            else this.load();
        });
    }
    ...
    save() {
        fs.writeFile(this.configFile, JSON.stringify(this.config), 'utf8', (err) => {
            if (err) logger.log('error', "Write error.", err);
                ...
                logger.log('debug', "Config file has been saved: %s", this.configFile);
    ...
    load() {
        fs.readFile(this.configFile, 'utf8', (err, data) => {
            if (err) logger.log('error', "Read error.", err);
                ...
                logger.log('debug', "Config file has been loaded: %s", this.configFile);
            }
        });
    }
